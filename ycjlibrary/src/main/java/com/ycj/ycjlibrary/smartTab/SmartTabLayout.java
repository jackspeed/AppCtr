...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    private static final int TAB_VIEW_MARGIN_DIPS = 20;
    private static final boolean TAB_VIEW_TEXT_ALL_CAPS = true;
    private static final int TAB_VIEW_TEXT_SIZE_SP = 12;
    private static final int TAB_VIEW_TEXT_SIZE_SL_SP = 12;//选择的textView的字体大小
    ...
    private float tabViewTextSelectorSize;
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        float slTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
                TAB_VIEW_TEXT_SIZE_SL_SP, dm);
        int textHorizontalPadding = (int) (TAB_VIEW_PADDING_DIPS * density);
        int textHorizontalMargin = (int) (TAB_VIEW_MARGIN_DIPS * density);
        ...
        textAllCaps = a.getBoolean(R.styleable.stl_SmartTabLayout_stl_defaultTabTextAllCaps,
                textAllCaps);
        ...
        slTextSize = a.getDimension(R.styleable.stl_SmartTabLayout_stl_selectorTabTextSize,
                slTextSize);
        ...
        textHorizontalMargin = a.getDimensionPixelSize(
                R.styleable.stl_SmartTabLayout_stl_defaultTabTextHorizontalMargin,
        ...
        distributeEvenly = a.getBoolean(R.styleable.stl_SmartTabLayout_stl_distributeEvenly,
                distributeEvenly);
        clickable = a.getBoolean(R.styleable.stl_SmartTabLayout_stl_clickable, clickable);
        titleOffset = a.getLayoutDimension(R.styleable.stl_SmartTabLayout_stl_titleOffset,
                titleOffset);
        a.recycle();
        ...
        this.tabViewTextSelectorSize = slTextSize;
    ...
    public void setCustomTabView(int layoutResId, TextView textViewId) {
        tabProvider = new SimpleTabProvider(getContext(), layoutResId, textViewId);
    }
    /**
     * Set the custom layout to be inflated for the tab views.
    ...
    protected TextView createDefaultTabView(CharSequence title, int position) {
        ...
        textView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                position == 0 ? tabViewTextSelectorSize : tabViewTextSize);
        ...
        params.setMargins(tabViewTextHorizontalMargin, 0, tabViewTextHorizontalMargin, 0);
     ...
     * {@link #setCustomTabView(int, int)}.
     */
    protected TextView createDefaultTabView(CharSequence title, int position, TextView textView) {
        textView.setText(title);
        textView.setTextColor(tabViewTextColors);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                position == 0 ? tabViewTextSelectorSize : tabViewTextSize);
    ...
    private void populateTabStrip() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            final View tabView;
            if (textView != null) {
                tabView = createDefaultTabView(adapter.getPageTitle(i), i, textView);
            } else if (tabProvider == null) {
                tabView = createDefaultTabView(adapter.getPageTitle(i), i);
            } else {
                tabView = tabProvider.createTabView(tabStrip, i, adapter);
            }
//            final View tabView = (tabProvider == null) ? createDefaultTabView(
//                    adapter.getPageTitle(i), i) : tabProvider.createTabView(tabStrip, i, adapter);
            if (tabView == null) {
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter()) {
            ...
            if (0f < positionOffset && positionOffset < 1f) {
                ...
                int selectHalfWidth = Utils.getWidth(selectedTab) / 2 +
                        Utils.getMarginEnd(selectedTab);
        ...
        if (isLayoutRtl) {
            x += start + startMargin - extraOffset - getWidth() +
                    Utils.getPaddingHorizontally(this);
        } else {
    ...
    /**
     * Interface definition for a callback to be invoked when the scroll position of a view
     * changes.
     */
    ...
    private static class SimpleTabProvider implements TabProvider {
        ...
        private int tabViewTextViewId=0;
        ...
        private SimpleTabProvider(Context context, int layoutResId, TextView t) {
            inflater = LayoutInflater.from(context);
            tabViewLayoutId = layoutResId;
            tv = t;
        }
    ...
    private class InternalViewPagerListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            ...
            for (int i = 0, size = tabStrip.getChildCount(); i < size; i++) {
                TextView tmpView = (TextView) tabStrip.getChildAt(i);
                tmpView.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabViewTextSize);
                tabStrip.getChildAt(i).setSelected(position == i);
            }
            //设置选中的item的字体大小
            TextView tmpView = (TextView) tabStrip.getChildAt(position);
            tmpView.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabViewTextSelectorSize);
            if (viewPagerPageChangeListener != null) {
