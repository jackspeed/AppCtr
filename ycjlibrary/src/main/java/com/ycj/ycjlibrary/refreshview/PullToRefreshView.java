...
//			Toast.makeText(mContext, this.msg_read_themes_failed, Toast.LENGTH_SHORT).show();
    ...
    private void initStyle(TypedArray typedArray) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (attr == R.styleable.PullToRefreshViewAttr_mode) {
                this.mode = typedArray.getInt(attr, MODE_PULL_DOWN_TO_REFRESH);
            } else if (attr == R.styleable.PullToRefreshViewAttr_header_background_color) {
                headerBackgroundColor = typedArray.getColor(attr, getResources().getColor(R.color.white));
            } else if (attr == R.styleable.PullToRefreshViewAttr_header_background_resource) {
                headerBackgroundResource = typedArray.getResourceId(attr, 0);
            } else if (attr == R.styleable.PullToRefreshViewAttr_content_background_color) {
                contentBackgroundColor = typedArray.getColor(attr, getResources().getColor(R.color.white));
            } else if (attr == R.styleable.PullToRefreshViewAttr_content_background_resource) {
                contentBackgroundResource = typedArray.getResourceId(attr, 0);
            } else if (attr == R.styleable.PullToRefreshViewAttr_footer_background_color) {
                footerBackgroundColor = typedArray.getColor(attr, getResources().getColor(R.color.white));
            } else if (attr == R.styleable.PullToRefreshViewAttr_footer_background_resource) {
    ...
    public void addLoadingView() {
        ...
        if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
            headerLayout = new LoadingLayout(context,
                    MODE_PULL_DOWN_TO_REFRESH, true, releaseDownLabel, pullDownLabel,
                    refreshingDownLabel);
        ...
        if (mode == MODE_PULL_UP_TO_LOAD_MORE || mode == MODE_BOTH) {
            footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_LOAD_MORE, true,
...
//                Log.e("Intercept", "move");
                if (isReadyForPull()) {
                    ...
                    if (yDiff > touchSlop && yDiff > xDiff) {
                        ...
                        } else if ((mode == MODE_PULL_UP_TO_LOAD_MORE || mode == MODE_BOTH)
                                && dy <= 0.0001f && isReadyForPullUp()) {
                        /*可以上拉更多*/
                            lastMotionY = y;
                            isBeingDragged = true;
                            currentMode = MODE_PULL_UP_TO_LOAD_MORE;
    ...
    public boolean isReadyForPullDown() {
        if (getChildCount() > 1) {
            ...
            if (childView instanceof ListView) {
                ...
                RecyclerView mListView = (RecyclerView) childView;
                ...
                GridLayoutManager gridLManager = null;
                LinearLayoutManager linearLManager = null;
                if (layoutManager instanceof GridLayoutManager) {
                    gridLManager = (GridLayoutManager) layoutManager;
                } else {
                    linearLManager = (LinearLayoutManager) layoutManager;
                }


                int firstVisiblePosition;
                if (gridLManager != null) {
                    firstVisiblePosition = gridLManager.findFirstVisibleItemPosition();
                } else {
                    firstVisiblePosition = linearLManager.findFirstVisibleItemPosition();
                }
...
//                if (((ViewGroup) childView).getScrollY() == 0) {
//                    return true;
//                } else {
//                    return false;
//                }
    ...
    public boolean isReadyForPullUp() {
        if (getChildCount() > 1) {
            ...
            if (childView instanceof ListView) {
                ...
                if (gridLManager != null) {
                    lastVisiblePosition = gridLManager.findLastVisibleItemPosition();
                } else {
                    lastVisiblePosition = linearLManager.findLastVisibleItemPosition();
                }

                if (lastVisiblePosition >= lastItemPosition - 1) {
                    final int childIndex = lastVisiblePosition - firstVisiblePosition;
                    final View lastVisibleChild = layoutManager.getChildAt(childIndex);
                    if (lastVisibleChild != null) {
                        return lastVisibleChild.getBottom() <= mListView.getBottom();
                    }
                }
...
//            else if (childView instanceof ViewGroup) {
//                int off = childView.getScrollY()
//                        + childView.getHeight()
//                        - ((ViewGroup) childView).getChildAt(0).getHeight();
//                return off >= 0;
//            }
